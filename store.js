...
function buildTimelineSeries(machineId, baseEnpi, basePowerKw) {
  ...
  for (let index = 23; index >= 0; index -= 1) {
    ...
    const kva = Number((powerKw / 0.85 + (Math.random() - 0.5) * 10).toFixed(1));
    const cosPhi = Number((0.92 + (Math.random() - 0.5) * 0.05).toFixed(3));
    const thdVoltage = Number((3.2 + (Math.random() - 0.5) * 1.5).toFixed(1));
    const oee = Math.round(clamp(78 + trend * 20 + (Math.random() - 0.5) * 8, 45, 95));
    const piecesCount = Math.round(120 + trend * 50 + (Math.random() - 0.5) * 20);
    const tonnage = Number((piecesCount * 0.25 + (Math.random() - 0.5) * 5).toFixed(2));
